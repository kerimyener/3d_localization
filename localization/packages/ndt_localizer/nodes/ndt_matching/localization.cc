...
void core::param_callback(ros::NodeHandle nh){
...
void core::odomPub (const ros::TimerEvent& event){
    tf::TransformBroadcaster odom_broadcaster;
    geometry_msgs::Quaternion laser_odom_quat = tf::createQuaternionMsgFromYaw(current_pose.yaw);
    ...
    laser_odom.header.stamp = current_scan_time;
    laser_odom.header.frame_id = "odom";

    //set the position
    laser_odom.pose.pose.position.x = current_pose.x;
    laser_odom.pose.pose.position.y = current_pose.y;
    laser_odom.pose.pose.position.z = 0.0;
    laser_odom.pose.pose.orientation = laser_odom_quat;

    //set the velocity
    laser_odom.child_frame_id = "base_link";
    laser_odom.twist.twist.linear.x = current_velocity;
    laser_odom.twist.twist.linear.y = 0.0;
    laser_odom.twist.twist.angular.z = angular_velocity;
...
void core::toInitNdt(){
  ...
  odom_pub = nh.advertise<nav_msgs::Odometry>("odometry/laser", 100000);
  ros::Timer odomTimer = nh.createTimer(ros::Duration(1.0/10.0/*10 Hz*/), &core::odomPub, this);
  // Subscribers
  param_callback(nh);
  //ros::Subscriber param_sub = nh.subscribe("config/ndt", 10, param_callback);
  ros::Subscriber gnss_sub = nh.subscribe("gnss_pose", 10, &core::gnss_callback, this);
  ros::Subscriber map_sub = nh.subscribe("points_map", 1, &core::map_callback, this);
  ros::Subscriber initialpose_sub = nh.subscribe("initialpose", 10, &core::initialpose_callback, this);
  ros::Subscriber points_sub = nh.subscribe("filtered_points", _queue_size, &core::points_callback, this);
  ros::Subscriber odom_sub = nh.subscribe("/odom_pose", _queue_size * 10, &core::odom_callback,this);
  ros::Subscriber imu_sub = nh.subscribe(_imu_topic.c_str(), _queue_size * 10, &core::imu_callback, this); 
  ros::spin();
  }
}
