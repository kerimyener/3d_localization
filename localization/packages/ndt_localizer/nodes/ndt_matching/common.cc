...
void core::points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
    if (map_loaded == 1 && init_pos_set == 1)
    {
        ...
        if (_use_imu == true && _use_odom == true)
            this->imu_odom_calc(current_scan_time);
        if (_use_imu == true && _use_odom == false)
            this->imu_calc(current_scan_time);
        if (_use_imu == false && _use_odom == true)
            this->odom_calc(current_scan_time);
    ...
    */
        ...
        this->odomPub(current_scan_time, current_pose.x, current_pose.y, current_pose.yaw, current_velocity_x, current_velocity_y, angular_velocity);
